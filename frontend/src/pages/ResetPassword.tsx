...
const ResetPassword: React.FC = () => {
  ...
  const [modo, setModo] = useState<"token" | "sesion" | "error">("error");
  ...
  useEffect(() => {
    if (token) {
      ...
      setModo("error");
      setMensaje("Token no válido o sesión inexistente.");
    }
  }, [token, isAuthenticated, sessionToken]);
    ...
    try {
      const url =
        modo === "token"
          ...
          : "http://localhost:8000/usuarios/me/reset-password-direct";

      const headers: Record<string, string> = {
        "Content-Type": "application/json",
      };

      if (modo === "sesion" && sessionToken) {
        headers["Authorization"] = `Bearer ${sessionToken}`;
      }

      const body =
        modo === "token"
          ? JSON.stringify({ token: token, nueva_contrasena: password })
          : JSON.stringify({
