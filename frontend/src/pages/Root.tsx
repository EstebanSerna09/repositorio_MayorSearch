...
export default function HomeRoot() {
  ...
  return (
    <div className="root-isolated min-h-screen bg-white text-blue-900 flex flex-col">
      {/* Header */}
      <header className="w-full bg-white text-blue-800 shadow-sm relative z-50">
  <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
    ...
    <div className="flex items-center shrink-0">
      <Link to="/">
        <img
          ...
          className="w-36 sm:w-40 object-contain cursor-pointer"
        />
      </Link>
    </div>

    {/* Botón hamburguesa (tablet/móvil) */}
    <button
      className="xl:hidden text-blue-900 focus:outline-none"
    ...
    {/* Nav escritorio */}
    <nav className="hidden xl:flex items-center gap-10 text-sm">
      ...
      <details className="relative group">
        ...
        <div className="absolute left-0 mt-2 w-56 bg-white border border-blue-100 rounded-lg shadow-lg z-50 overflow-hidden group-open:block hidden">
      ...
      <Link to="/home" className="text-blue-700/80 hover:text-blue-800 transition-colors">
        BUSCAR
      </Link>

      <a href="#contacto" className="text-blue-700/80 hover:text-blue-800 transition-colors">
        CONTÁCTANOS
      </a>
    </nav>

    {/* UserMenu (solo en escritorio grande) */}
  ...
  <div
    className={`xl:hidden absolute top-full left-0 w-full bg-white shadow-md border-t border-blue-100 py-4 transition-all duration-300 ease-in-out ${
      menuOpen
  ...
  >
    <div className="flex flex-col items-center gap-4 text-blue-800">
      ...
      <Link to="/explorar" onClick={() => setMenuOpen(false)} className="hover:text-blue-600">
        Explorar
      </Link>
      <Link to="/autores" onClick={() => setMenuOpen(false)} className="hover:text-blue-600">
        Autores
      </Link>
      <Link to="/teoriadegrafos" onClick={() => setMenuOpen(false)} className="hover:text-blue-600">
        Teoría de grafos
      </Link>
      <Link to="/analisisnumerico" onClick={() => setMenuOpen(false)} className="hover:text-blue-600">
        Análisis numérico
      </Link>
      <Link to="/home" onClick={() => setMenuOpen(false)} className="hover:text-blue-600">
        Buscar
      </Link>
      <a href="#contacto" onClick={() => setMenuOpen(false)} className="hover:text-blue-600">
        Contáctanos
      </a>
      <div className="mt-2">
        <UserMenu />
